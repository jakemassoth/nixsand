...
pub fn ensure_base_image(container: &dyn ContainerBackend) -> Result<()> {
    ...
    if container.image_exists(tag)? {
        eprintln!("[image] base image '{}' already exists, skipping build", tag);
        return Ok(());
    }
    ...
    eprintln!("[image] base image '{}' built successfully", tag);
    Ok(())
}
...
pub fn ensure_project_image(
    ...
    let needs_build = if !container.image_exists(&tag)? {
        eprintln!("[image] per-project image '{}' not found, building...", tag);
        true
    } else {
        match &recorded_hash {
            None => {
                eprintln!(
                    "[image] no flake.lock hash recorded for '{}', rebuilding image...",
                    project
                );
                true
            }
            Some(recorded) if recorded != &current_hash => {
                eprintln!(
                    "[image] flake.lock changed for '{}', rebuilding per-project image...",
                    project
                );
                true
            }
            _ => {
                eprintln!(
                    "[image] per-project image '{}' is up to date, skipping build",
                    tag
                );
                false
            }
    ...
    eprintln!("[image] per-project image '{}' built successfully", tag);
    Ok(())
}
